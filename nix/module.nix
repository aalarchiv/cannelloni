...
in
{
  options.services.cannellonis = with lib; {
    ...
    mdns = mkOption {
      ...
      description = ''
        ...
        peerTimeout apply. Automatically enables a publishing services.avahi
        (override services.avahi.* to customise). UDP only; off by default.
        Trusted networks only -- unauthenticated by design.
      '';
    };
    maxPeers = mkOption {
      type = types.int;
  ...
  config = lib.mkIf cfg.enable {
    # mDNS discovery publishes + browses via the Avahi *client* API, which the
    # daemon refuses unless publishing is enabled (it sets disable-publishing=yes
    # otherwise, which kills our client entry group too). So when mdns is on,
    # bring up a publishing avahi-daemon by default; mkDefault lets the operator
